...
#include <string.h> // pour traitements de caracteres
#include <unistd.h> //pour types de variables
#include <termios.h> // pour port serie
#include "/usr/include/mysql/mysql.h"
int Fd; // Ouverture associera port avec ce File Descriptor
struct termios MaConf; // Config du port

void main () {
        MYSQL *conn;
        char requete[80];
	conn = mysql_init(NULL);
        mysql_real_connect(conn,"Localhost","pi","GabinArdyAG23.","BD_SAE301",0,NULL,0);
        ...
        printf("\nTempérature: %d\n", Val3);
        if (Val3 > 24) {
                system("/home/pi/script_appel.sh");
        }
